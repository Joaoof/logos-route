...
export function ScreenAuth({ type, onSubmit, isLoading = false, error = null, usarApi = false }: ScreenAuthProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await onSubmit({ nome, email, senha, cidade, appUtilizado })
  }
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            {(!usarApi || type === "cadastro") && (
              <div className="flex items-center bg-white rounded-full px-4 h-12 shadow-sm">
                ...
                <input id="nome" type="text" placeholder="NOME" value={nome} onChange={(e) => setNome(e.target.value)} required className="w-full bg-transparent outline-none text-[#1ce4d2] font-bold text-xs tracking-wider ml-3 placeholder:text-[#1ce4d2]/60" />
              </div>
            )}
            ...
            {usarApi && (
              <div className="flex items-center bg-white rounded-full px-4 h-12 shadow-sm">
                ...
                <input id="email" type="email" placeholder="E-MAIL" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full bg-transparent outline-none text-[#1ce4d2] font-bold text-xs tracking-wider ml-3 placeholder:text-[#1ce4d2]/60" />
              </div>
            )}
            ...
            {usarApi && (
              <div className="flex items-center bg-white rounded-full px-4 h-12 shadow-sm">
                ...
                <input id="senha" type="password" placeholder="SENHA" value={senha} onChange={(e) => setSenha(e.target.value)} required className="w-full bg-transparent outline-none text-[#1ce4d2] font-bold text-xs tracking-wider ml-3 placeholder:text-[#1ce4d2]/60" />
              </div>
            )}
            ...
            {usarApi && type === "cadastro" && (
              <div className="flex items-center bg-white rounded-full px-4 h-12 shadow-sm">
                ...
                <input id="confirmarSenha" type="password" placeholder="CONFIRMAR SENHA" value={confirmarSenha} onChange={(e) => setConfirmarSenha(e.target.value)} required className="w-full bg-transparent outline-none text-[#1ce4d2] font-bold text-xs tracking-wider ml-3 placeholder:text-[#1ce4d2]/60" />
              </div>
            )}

            <button type="submit" disabled={isLoading} className="mt-4 mx-auto bg-white text-[#1ce4d2] font-extrabold tracking-widest text-[11px] px-8 py-3 rounded-full shadow-md hover:scale-105 active:scale-95 transition-transform flex items-center justify-center min-w-[120px]" aria-label={type === "cadastro" ? "Enviar cadastro" : "Enviar login"}>
